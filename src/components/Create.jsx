...
import { Modal, Button, Popconfirm, ConfigProvider } from "antd";
...
import { modifyUserCTA, modifyUserData, modifyUserDetails, modifyUserSocial } from "../utility/dataModify"

const Create = () => {
	const beforeUnloadWarning = useRef(true);
	useBeforeunload((event) => {
		if (beforeUnloadWarning.current === true) {
	...
	const [userPopConfirmed, setUserPopConfirmed] = useState(false);
	...
	const [socialCounter, setSocialCounter] = useState(["facebook", "instagram", "twitter", "linkedIn", "youtube"]);
	const [whatsAppUrl, setWhatsAppUrl] = useState('');
	const { enqueueSnackbar } = useSnackbar();
	const [warning, setWarning] = useState({
		template: { status: false, message: "" },
		name: { status: false, message: "" },
		designation: { status: false, message: "" },
		email: { status: false, message: "" },
		social: { status: false, message: "" }
	});
	...
	function handleCheckBox(e, name) {
		if (e.target.checked) {
			if (socialCounter?.length < 5) {
				setSocialChecker(prev => ({ ...prev, [name]: true }));
				setSocialCounter(prev => ([...prev, name]));
			} else {
				enqueueSnackbar("Only Five Social Links allowed", { variant: "customWarning" });
				setSocialChecker(prev => ({ ...prev, [name]: false }));
			}
		} else {
			setSocialChecker(prev => ({ ...prev, [name]: false }));
			let temp = socialCounter?.filter(item => (item !== name));
			setSocialCounter(temp);
		}
	}
	...
	const handleRemoveImage = () => {
		try {
			...
			setImage(null);
			setFile(null);
			setUserData(prev => ({ ...prev, profileImage: 'avatar.png', visited: { ...prev.visited, profile: false } }));
			setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, profileImage: 'avatar.png', visited: { ...prev.userData?.visited, profile: false } } }));
		} catch (error) {
			console.log(error);
		}
	}
	...
	function handleFinishRedirect() {
		try {
			...
			if (!loggerData?.isLoggedIn) {
				...
				let modifiedUserData = modifyUserData(userData, socialChecker);
				let modifiedUserCTA = modifyUserCTA(userData, setCtaDisplay);
				setLoggerData(prev => ({ ...prev, userData: { ...modifiedUserData, ...modifiedUserCTA } }));
				setUserData(prev => ({ ...prev, ...modifiedUserCTA, ...modifiedUserData }));
				userServices.addUserData(loggerData?.user_id, loggerData?.email, loggerData?.access_token, { ...modifiedUserData, ...modifiedUserCTA }, chosenTemplateData?._id)
					.then(res => { })
					.catch((err) => { });
				if (sign_ref.current) {
				...
				setTimeout(() => {
					setLoading(false);
					navigate("/finish", { state: { template_data: sign_ref?.current } });
				}, 2000)
			}
		} catch (error) {
			setLoading(false);
		...
		replace: domNode => {
			try {
				...
				switch (domNode.attribs && domNode.attribs.id) {
					...
					case ('banner-blk'):
						props = attributesToProps(domNode.attribs);
						if (visited?.banner || ctaDisplay?.banner || userData?.banner) {
							props.class = 'animate__animated animate__fadeIn'
							props.style.display = 'table-row'
						} else {
							props.style.display = 'none'
						}
					...
					case ('social-container'):
						props = attributesToProps(domNode.attribs);
						if (!socialChecker?.facebook && !socialChecker?.youtube && !socialChecker?.instagram && !socialChecker?.twitter && !socialChecker?.linkedIn && !socialChecker?.skype && !socialChecker?.whatsapp && !socialChecker?.pinterest) {
							props.style.display = 'none'
						}
						else {
							props.style.display = 'table-row'
					...
					case ('website-blk'):
						...
						return <span {...props}>{domToReact(domNode.children, options)}</span>
					case ('apple-app-link-blk'):
						props = attributesToProps(domNode.attribs);
						if (userData.appleStoreAppLink || ctaDisplay?.appLink) {
							props.class = 'animate__animated animate__fadeIn'
					...
					case ('playstore-app-link-blk'):
						props = attributesToProps(domNode.attribs);
						if (userData.playStoreAppLink || ctaDisplay?.appLink) {
							props.class = 'animate__animated animate__fadeIn'
							props.style.display = 'table-cell'
						} else {
							props.style.display = 'none'
					...
					case ('profile-pic'):
						if (userData.profileImage) {
							...
							domNode.attribs.src = process.env.REACT_APP_BACKEND_URL + '/profile/' + 'avatar.png';
						}
						props = attributesToProps(domNode.attribs);
					...
					case ('link-whatsapp'):
						if (userData.whatsapp) {
							domNode.attribs.href = userData.whatsapp.startsWith('placeholder') ? '#' : `http://wa.me/${userData?.whatsapp}`;
	...
	const validateTabberChange = (tabData, editMode) => {
		...
		if (editMode === true) {
			...
			if (tabData === 'filter-social') {
				...
				if (!chosenTemplateId) {
					setWarning(prev => ({ ...prev, template: { status: true, message: "" } }));
					return enqueueSnackbar('Please Choose a Template', {
				...
				if (allow === 3) {
					setTabber(tabData);
					setPercent(75);
					let modifiedUserDetails = modifyUserDetails(userData);
					setWarning(prev => ({ ...prev, template: { status: false, message: "" } }));
					setUserData(prev => ({ ...prev, ...modifiedUserDetails, visited: { ...prev.visited, details: true, social: true } }));
					setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, ...modifiedUserDetails, visited: { ...prev.visited, details: true, social: true } } }));
					!userPopConfirmed && setOpen(true);
				}
				if (errorFlag) {
					setTabber("filter-details");
			...
			if (tabData === 'filter-cta') {
				...
				/** END EMAIL VALIDATION */
				if (allow === 3) {
					if ((userData?.visited?.social || userData?.facebook || userData?.linkedIn || userData?.twitter || userData?.instagram || userData?.youtube || userData?.skype || userData?.whatsapp || userData?.pinterest)) {

						setTabber(tabData);
						setPercent(100);
						let modifiedUserData = modifyUserSocial(userData, socialChecker, setSocialChecker, socialCounter, setSocialCounter);
						setWarning(prev => ({ ...prev, template: { status: false, message: "" }, social: { status: false, message: "" } }));
						setVisited(prev => ({ ...prev, social: true, cta: true }));
						setUserData(prev => ({ ...prev, ...modifiedUserData, visited: { ...prev.visited, details: true, social: true, cta: true } }));
						setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, ...modifiedUserData, visited: { ...prev.visited, details: true, social: true, cta: true } } }));
					} else {
						return setWarning(prev => ({ ...prev, social: { status: true, message: "Please edit your social links" } }));
					}
				}
				if (errorFlag) {
					setTabber("filter-details");
	...
	function chooseTemplate(template) {
		setWarning(prev => ({ ...prev, template: { status: false, message: "" } }));
		setLoggerData(prev => ({ ...prev, template_id: template }));
		setChosenTemplateId(template);
	}

	useEffect(() => {
		if (state?.template_id) {
			chooseTemplate(state?.template_id);
		} else if (state?.chosen_draft_template_id) {
			chooseTemplate(state?.chosen_draft_template_id);
		} else if (loggerData?.template_id) {
			chooseTemplate(loggerData?.template_id);
		}
		...
		try {
			if (templates.length > 0) {
				...
				if (match) {
					...
					if (loggerData?.isLoggedIn) {
						if (state?.chosen_draft_userData && !userData?.fullName && !userData?.email && !userData?.designation) {
							...
							if (loggerData?.userData?.email && loggerData?.userData?.fullName && loggerData?.userData?.designation) {
								setUserData(prev => ({ ...prev, ...loggerData?.userData }));
							} else {
	...
	async function handleImageUpload(data) {
		...
		try {
			...
			if (error?.response?.data?.message === "File upload error" && error?.response?.data?.error === "Field value too long") {
				enqueueSnackbar("Image uploaded is greater than the 5MB limit.", { variant: "customError" })
			} else {
			...
			setUserData(prev => ({ ...prev, profileImage: "avatar.png", visited: { ...prev.visited, profile: false } }));
			setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, profileImage: "avatar.png", visited: { ...prev.visited, profile: false } } }));
		}
	}
	...
	async function youtube_parser(url) {
		...
		if (url !== "" || !match) {
			if (validator.isURL(url)) {
				// const data = await axios.get(`https://www.googleapis.com/youtube/v3/videos?part=snippet&id=${match[7]}&key=${process.env.REACT_APP_YOUTUBE_API_KEY}`);
				// if(data){
				// 	console.log("DATA FROM YOUTUBE PARSER", data);
				// 	setUserData(prev => ({ ...prev, video: url, video_img: data?.items[0]?.snippet?.thumbnails?.default?.url, video_title: data?.items[0]?.snippet?.title, video_description:data?.items[0]?.snippet?.description }));
				// 	setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, youtube: url, video_img: data?.items[0]?.snippet?.thumbnails?.default?.url, video_title: data?.items[0]?.snippet?.title, video_description:data?.items[0]?.snippet?.description } }));
				// }
				fetch(`https://noembed.com/embed?dataType=json&url=${url}`)
					...
					.then(data => {
						...
						setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, youtube: url, video_img: `https://i.ytimg.com/vi/${match}/default.jpg`, video_title: title } }));
					}).catch(err => console.log(err))
			} else {
				setUserData(prev => ({ ...prev, video: url, video_img: '', video_title: '', video_description:'' }));
				setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, video: url, video_img: '', video_title: '', video_description:'' } }));
			}
		} else {
			setUserData(prev => ({ ...prev, video: "", video_img: "", video_title: "", video_description:'' }));
			setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, video: "", video_img: "", video_title: "", video_description:'' } }));
		}
	...
	function loginAndRedirect() {
		try {
			...
			setTimeout(() => {
				setLoading(false);
				sessionStorage.setItem("userData", JSON.stringify({ ...modifiedUserData, ...modifiedUserCTA }));
				sessionStorage.setItem("template_data", sign_ref.current);
				sessionStorage.setItem("template_id", chosenTemplateData?._id);
				sessionStorage.setItem("gotofinish", "true");
				navigate('/login', { state: { redirectToCreate: true, modifiedUserData: { ...modifiedUserData, ...modifiedUserCTA }, template_id: chosenTemplateData?._id, template_data: sign_ref.current } });
			}, 2000);
		} catch (error) {
			setLoading(false);
			console.log(error);
	...
	function handleCtaDisplay(uuid) {
		try {
			console.log("HANDLE CTA ACCORDION", uuid)
			if (uuid.length > 0) {
	...
	useEffect(() => {
		fetchChosenTemplate(chosenTemplateId);
	}, [chosenTemplateId, loggerData?.isLoggedIn])
	...
	useEffect(() => {
		if (userData) {
			...
			} else if (userData?.facebook || (userData?.facebook === null && initialValues?.facebook)) {
			...
			} else if (userData?.twitter || (userData?.twitter === null && initialValues?.twitter)) {
			...
			} else if (userData?.linkedIn || (userData?.linkedIn === null && initialValues?.linkedIn)) {
			...
			} else if (userData?.instagram || (userData?.instagram === null && initialValues?.instagram)) {
			...
			} else if (userData?.youtube || (userData?.youtube === null && initialValues?.youtube)) {
			...
			} else if (userData?.skype || (userData?.skype === null && initialValues?.skype)) {
			...
			} else if (userData?.whatsapp || (userData?.whatsapp === null && initialValues?.whatsapp)) {
			...
			else if (userData?.pinterest || (userData?.pinterest === null && initialValues?.pinterest)) {
				setSocialChecker(prev => ({ ...prev, pinterest: true }));
				if (arr?.length < 5) {
					arr.push("twitter");
				}
			}
			setSocialCounter(arr);
		}
	}, [userData?.fullName])

	useEffect(() => {
		if (userData?.video) {
			youtube_parser(userData?.video);
		}
	...
	return (
		<ErrorBoundary FallbackComponent={<FallBack />}>
			<>
				...
				<Header source={'create'} userData={userData} initialValues={initialValues} template_id={chosenTemplateId} template_data={sign_ref.current} socialChecker={socialChecker} setCtaDisplay={setCtaDisplay} />
				<section className="create-template-wr">
					<div className="center-wr">
						<div className='heading-container for-cursive' style={{ display: "flex", alignItems: 'center', justifyContent: 'center', padding: "0px 25px 40px" }}>
							{tabber === "filter-template" ? <h4><span>Choose a Template.</span> We have an exciting collection of templates</h4> : tabber === "filter-details" ? <h4><span>Please fill your details.</span> Your Name, designation, and email address are required for best experience.</h4> : tabber === "filter-social" ? <h4><span>Stay Connected!</span> Enter your social links.</h4> : <h4><span>Finishing Touches.</span> Add CTA and other details.</h4>}
						</div>
						<div className="create-template-content clearfix">
							<Progress className='progressResponsive' success={{ strokeColor: "green" }} format={(percent) => (`Progress: ${percent}%. Steps ${percent / 25}/4 Completed.`)} strokeColor={'#625BF8'} percent={percent} />
							<div id="hidn-tbbr" className="create-template-left">
								<div className="create-template-box create-box-blk" data-filter="filter-template" onClick={(e) => { validateTabberChange('filter-template', true) }}>
									<div className="create-template-box-count" style={{ backgroundColor: Object.values(chosenTemplateData)?.length > 0 ? '#71dd37' : warning?.template?.status ? "#F84525" : "#625bf8" }}>{Object.values(chosenTemplateData)?.length > 0 ? <img src='assets/images/tick-svgrepo-com.svg' alt="" width={24} height={24} /> : warning?.template?.status ? "!!" : '1'}</div>
								...
								<div className="create-details-box create-box-blk" style={{ backgroundColor: tabber === "filter-details" && "#6440FB" }} data-filter="filter-details" onClick={(e) => { validateTabberChange('filter-details', true) }}>
									<div className="create-details-box-count"
										style={{
											backgroundColor: ((userData?.fullName && userData?.email && isEmail(userData?.email) && userData?.designation)) ? '#71dd37' : (warning?.designation?.status || warning?.email?.status || warning?.name?.status) ? "#F84525" : "#625bf8"
										}}>
										{((userData?.fullName && userData?.email && isEmail(userData?.email) && userData?.designation)) ? <img src='assets/images/tick-svgrepo-com.svg' alt="" width={24} height={24} /> : (warning?.designation?.status || warning?.email?.status || warning?.name?.status) ? "!!" : '2'}
									</div>
									<div className="create-template-box-inner">
										<div className="create-box-icon">
								...
								<Popconfirm
									title={<div style={{ display: "flex", alignItems: 'center', justifyContent: "start", gap: "15px" }}><FiHelpCircle style={{ color: "orange", fontSize: "18px" }} /><p>Hey! Just a heads up.</p></div>}
									description={<p>You can choose any 5 social links.</p>}
									onConfirm={confirm}
									okText="Okay"
									open={open}
									placement='left'
									onOpenChange={(newopen) => { }}
									okButtonProps={{ style: { backgroundColor: "#625bf8", padding: "10px", borderRadius: "5px" } }}
									cancelButtonProps={{ style: { display: "none" } }}
									icon={<></>}
								>
									<div className="create-social-box create-box-blk" data-filter="filter-social" onClick={(e) => { validateTabberChange('filter-social', true) }}>
										<div className="create-social-box-count"
											style={{ backgroundColor: (userData?.visited?.social || userData?.facebook || userData?.linkedIn || userData?.twitter || userData?.instagram || userData?.youtube || userData?.skype || userData?.whatsapp || userData?.pinterest) ? '#71dd37' : warning?.social?.status ? '#F84525' : '#625bf8' }}
										>
											{(userData?.visited?.social || userData?.facebook || userData?.linkedIn || userData?.twitter || userData?.instagram || userData?.youtube || userData?.skype || userData?.whatsapp || userData?.pinterest) ? <img src='assets/images/tick-svgrepo-com.svg' alt="" width={24} height={24} /> : warning?.social?.status ? "!!" : '3'}
										</div>
										<div className="create-template-box-inner">
											<div className="create-box-icon">
								...
								<div className="create-cta-box create-box-blk" data-filter="filter-cta" onClick={(e) => { validateTabberChange('filter-cta', true) }}>
									<div className="create-cta-box-count" style={{ backgroundColor: (visited?.cta || userData?.visited?.cta || userData?.quote || userData?.disclaimer || userData?.video || userData?.appleStoreAppLink || userData?.banner || userData?.playStoreAppLink || userData?.feedback) && '#71dd37' }}>{(visited?.cta || userData?.visited?.cta || userData?.quote || userData?.disclaimer || userData?.video || userData?.appleStoreAppLink || userData?.banner || userData?.playStoreAppLink || userData?.feedback) ? <img src='assets/images/tick-svgrepo-com.svg' alt="" width={24} height={24} /> : '4'}</div>
									<div className="create-template-box-inner">
										<div className="create-box-icon">
								...
								<div className="template-data-blk" id='template-data-blk-id' style={{ pointerEvents: 'none' }}>
									{
										sign_ref.current ? getSignature()
											...
											Object.keys(chosenTemplateData).length > 0 ?
												...
												:
												<div className='chooseSkeleton' style={{ width: "100%", height: "239px", backgroundColor: 'rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '20px' }}>
							...
							<div className="create-template-right tabber-content filter-template right">
												...
												>
													{
														...
														filteredTemplates?.map(item => (
															<SwiperSlide>
																<div key={item._id} onClick={() => { chooseTemplate(item?._id) }} style={{ padding: '0px', width: '100%', height: "auto" }} >
																	<div style={{
																		display: 'flex',
																		alignItems: 'center',
													...
													<ul>
														{
															filteredTemplates?.length > 0 &&
															filteredTemplates?.map(item => (
																<li key={item._id} onClick={() => { chooseTemplate(item?._id) }} style={{ padding: '0px', width: '100%', height: "auto" }} >
																	<div style={{
																		display: 'flex',
																		alignItems: 'center',
																		justifyContent: 'center',
								...
								<div className="some-variations-images"> */}
										...
										<div className="template-detail-btn">
											<span onClick={() => { validateTabberChange('filter-details', true); }} data-filter="filter-details" className="anchor-tabber site-btn-second">Next</span>
										</div>
									</div>
									<div style={{ justifyContent: 'flex-end', width: '100%' }} id='hidn-btn'>
										<Link onClick={() => { validateTabberChange('filter-details', true) }} className="site-btn" style={{ width: '30%', textAlign: 'center' }} >Next</Link>
									</div>
								</div>
							</div>
							<div className="create-template-right tabber-content filter-details right">
								<div className="create-template-right-inner">
									<div className="create-template-categories  animate__animated animate__fadeIn login-form">
										<form method="post" action="">
													...
													(image || userData?.profileImage) && (
														<div className="image-preview-blk">
															<div className="remove-image" onClick={handleRemoveImage}>
																<AiOutlineClose color="white" style={{ color: 'white', fontSize: '13px' }} />
															</div>
															<img onClick={() => { if (image) setIsModalOpen(true) }} src={image ? src : `${process.env.REACT_APP_BACKEND_URL}/profile/${userData?.profileImage}`} alt="preview" width={100} height={100} style={{ borderRadius: '10px', boxShadow: '4px 4px 12px 3px rgba(0,0,0,0.12)', objectFit: "cover" }} />
														</div>
													)
												}
											</div>
											...
											<div className="create-email form-field" style={{ display: chosenTemplateData ? chosenTemplateData?.filler?.email ? "block" : "none" : "block" }}>
												...
												<input
													id='email-required'
													placeholder="emailsig@gmail.com"
													style={{ color: warning?.email?.status ? "red" : "#000" }}
													onFocus={(e) => {
														if (warning?.email?.status) {
															setWarning(prev => ({ ...prev, email: { status: false, message: "" } }))
											...
											<div className="create-phone form-field" style={{ display: chosenTemplateData ? chosenTemplateData?.filler?.phone ? "block" : "none" : "block" }}>
												...
												<input placeholder="+801-1200-239-235" onFocus={(e) => { e.target.select() }} maxLength="20" onChange={(e) => { if (Object.keys(chosenTemplateData).length > 0) { setUserData(prev => ({ ...prev, phone: e.target.value?.replace(/[^0-9!@#$%^&*()-_+=]/g, '')?.slice(0, 20) })); setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, phone: e.target.value?.replace(/[^0-9!@#$%^&*()-_+=]/g, '')?.slice(0, 20) } })) } }} value={
													// logged in old user
											...
											<div className="create-website form-field" style={{ display: chosenTemplateData ? chosenTemplateData?.filler?.website ? "block" : "none" : "block" }}>
												...
												<input
													placeholder='www.bestwebsite.com'
													onFocus={(e) => { e.target.select() }}
													maxLength="100"
													onChange={(e) => {
														if (Object.keys(chosenTemplateData).length > 0) {
											...
											<div className="create-website form-field" style={{ display: chosenTemplateData ? chosenTemplateData?.filler?.location ? "block" : "none" : "block" }}>
												...
												<input placeholder='201 Lake View, USA' onFocus={(e) => { e.target.select() }} maxLength="100" onChange={(e) => { if (Object.keys(chosenTemplateData).length > 0) { setUserData(prev => ({ ...prev, location: e.target.value?.slice(0, 100) })); setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, location: e.target.value?.slice(0, 100) } })) } }}
													value={
									...
									<div style={{ justifyContent: 'space-between', width: '100%' }} id='hidn-btn'>
										<Link onClick={() => { validateTabberChange('filter-template', true) }} style={{ backgroundColor: '#FFF', color: '#6440FB', border: '1px solid rgba(0,0,0,0.6)', width: '30%', textAlign: 'center' }} className="site-btn">Back</Link>
										<Link onClick={() => { validateTabberChange('filter-social', true) }} className="site-btn" style={{ width: '30%', textAlign: 'center' }} >Next</Link>
									</div>
									...
									<div className="create-template-details">
										...
										<div className="template-detail-btn">
											<span onClick={() => { validateTabberChange('filter-template', true) }} data-filter="filter-social" className="anchor-tabber site-btn-second-reverse">Back</span>
											<span onClick={() => { validateTabberChange('filter-social', true); }} style={{ float: 'right' }} data-filter="filter-social" className="anchor-tabber site-btn-second">Next</span>
										</div>
									</div>
								</div>
							</div>
							<div className="create-template-right tabber-content filter-social right">
								<div className="create-template-right-inner">
									<div className="create-template-categories  animate__animated animate__fadeIn login-form">
										<form method="post" action="">
											<div className="create-name form-field contact-form-field checkboxed-input">
												<input type="checkbox" checked={socialChecker?.facebook} onChange={((e) => { handleCheckBox(e, "facebook") })} className="checker" id="facebook-check" name="facebook-check" value="facebook-selected" />
												...
												<input placeholder="https://www.facebook.com/username"
													...
													onChange={(e) => {
														if (Object.keys(chosenTemplateData).length > 0) {
															setUserData(prev => ({ ...prev, facebook: e.target.value?.slice(0, 100), visited: { ...prev.visited, social: true } }));
															setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, facebook: e.target.value?.slice(0, 100), visited: { ...prev.userData?.visited, social: true } } }));
														}
													}} type="text" name="create-facebook" autoComplete="off" />
											</div>
											<div className="create-email form-field contact-form-field checkboxed-input">
												<input type="checkbox" checked={socialChecker?.instagram} className="checker" onChange={((e) => { handleCheckBox(e, "instagram") })} id="instagram-check" name="instagram-check" value="instagram-selected" />
												...
												<input placeholder="https://www.instagram.com/username/"
													...
													onChange={(e) => {
														if (Object.keys(chosenTemplateData).length > 0) {
															setUserData(prev => ({ ...prev, instagram: e.target.value?.slice(0, 100), visited: { ...prev.visited, social: true } }));
															setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, instagram: e.target.value?.slice(0, 100), visited: { ...prev.userData?.visited, social: true } } }))
														}
											...
											<div className="create-website form-field contact-form-field checkboxed-input">
												<input type="checkbox" checked={socialChecker?.twitter} className="checker" onChange={((e) => { handleCheckBox(e, "twitter") })} id="twitter-check" name="twitter-check" value="twitter-selected" />
												...
												<input placeholder="https://twitter.com/username" onFocus={(e) => { e.target.select() }} maxLength="100"
													...
													onChange={(e) => {
														if (Object.keys(chosenTemplateData).length > 0) {
															setUserData(prev => ({ ...prev, twitter: e.target.value?.slice(0, 100), visited: { ...prev.visited, social: true } }));
															setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, twitter: e.target.value?.slice(0, 100), visited: { ...prev.userData?.visited, social: true } } }))
											...
											<div className="create-company form-field contact-form-field checkboxed-input">
												<input type="checkbox" checked={socialChecker?.linkedIn} className="checker" onChange={((e) => { handleCheckBox(e, "linkedIn") })} id="linkedin-check" name="linkedin-check" value="linkedin-selected" />
												...
												<input placeholder="https://www.linkedin.com/in/username"
													...
													onChange={(e) => {
														if (Object.keys(chosenTemplateData).length > 0) {
															setUserData(prev => ({ ...prev, linkedIn: e.target.value?.slice(0, 100), visited: { ...prev.visited, social: true } }));
															setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, linkedIn: e.target.value?.slice(0, 100), visited: { ...prev.userData?.visited, social: true } } }))

														}
													}} type="text" name="create-linkedin" autoComplete="off" />
											</div>
											<div className="create-phone form-field contact-form-field checkboxed-input">
												<input type="checkbox" checked={socialChecker?.youtube} className="checker" onChange={((e) => { handleCheckBox(e, "youtube") })} id="youtube-check" name="youtube-check" value="youtube-selected" />
												...
												<input onFocus={(e) => { e.target.select() }} maxLength="100" placeholder="https://www.youtube.com/watch?v=VIDEO_ID"
													...
													onChange={(e) => {
														if (Object.keys(chosenTemplateData).length > 0) {
															setUserData(prev => ({ ...prev, youtube: e.target.value?.slice(0, 100), visited: { ...prev.visited, social: true } }));
															setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, youtube: e.target.value?.slice(0, 100), visited: { ...prev.userData?.visited, social: true } } }))
											...
											<div className="create-phone form-field contact-form-field checkboxed-input">
												<input type="checkbox" className="checker" checked={socialChecker?.skype} onChange={((e) => { handleCheckBox(e, "skype") })} id="skype-check" name="skype-check" value="skype-selected" />
												...
												<input onFocus={(e) => { e.target.select() }} placeholder="live:skype_id" maxLength="100"
													...
													onChange={(e) => {
														if (Object.keys(chosenTemplateData).length > 0) {
															setUserData(prev => ({ ...prev, skype: e.target.value?.slice(0, 100), visited: { ...prev.visited, social: true } }));
															setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, skype: e.target.value?.slice(0, 100), visited: { ...prev.userData?.visited, social: true } } }))
											...
											<div className="create-phone form-field contact-form-field checkboxed-input">
												<input type="checkbox" className="checker" checked={socialChecker?.whatsapp} onChange={((e) => { handleCheckBox(e, "whatsapp") })} id="whatsapp-check" name="whatsapp-check" value="whatsapp-selected" />
												...
												<input onFocus={(e) => { e.target.select() }} placeholder="1234567890" maxLength="100"
													...
													onChange={(e) => {
														if (Object.keys(chosenTemplateData).length > 0) {
															setUserData(prev => ({ ...prev, whatsapp: e.target.value?.slice(0, 100), visited: { ...prev.visited, social: true } }));
															setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, whatsapp: e.target.value?.slice(0, 100), visited: { ...prev.userData?.visited, social: true } } }))
											...
											<div className="create-job-title form-field contact-form-field checkboxed-input">
												<input type="checkbox" className="checker" checked={socialChecker?.pinterest} onChange={((e) => { handleCheckBox(e, "pinterest") })} id="pinterest-check" name="pinterest-check" value="pinterest-selected" />
												...
												<input onFocus={(e) => { e.target.select() }} placeholder="https://www.pinterest.com/username/" maxLength="100"
													...
													onChange={(e) => {
														if (Object.keys(chosenTemplateData).length > 0) {
															setUserData(prev => ({ ...prev, pinterest: e.target.value?.slice(0, 100), visited: { ...prev.visited, social: true } }));
															setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, pinterest: e.target.value?.slice(0, 100), visited: { ...prev.userData?.visited, social: true } } }))
									...
									<div style={{ justifyContent: 'space-between', width: '100%' }} id='hidn-btn'>
										<Link onClick={(e) => { validateTabberChange('filter-details', true) }} style={{ backgroundColor: '#FFF', color: '#6440FB', border: '1px solid rgba(0,0,0,0.6)', width: '30%', textAlign: 'center' }} to="/create" className="site-btn">Back</Link>
										<Link onClick={(e) => { validateTabberChange('filter-cta', true) }} className="site-btn" style={{ width: '30%', textAlign: 'center' }} >Next</Link>
									</div>
									<div className="create-template-details">
										<div className="template-detail-top">
											<div className="template-top-circle">
										...
										<div className="template-detail-btn">
											<span onClick={() => { validateTabberChange('filter-details', true) }} data-filter="filter-cta" className="anchor-tabber site-btn-second-reverse">Back</span>
											<span onClick={() => { validateTabberChange('filter-cta', true) }} style={{ float: 'right' }} data-filter="filter-cta" className="anchor-tabber site-btn-second">Next</span>
										</div>
									</div>
								</div>
							</div>
							<div className="create-template-right tabber-content filter-cta right">
								<div className="create-template-right-inner">
									...
									<div style={{ justifyContent: 'space-between', width: '100%' }} id='hidn-btn'>
										<Link onClick={() => { validateTabberChange('filter-social', true) }} style={{ backgroundColor: '#FFF', color: '#6440FB', border: '1px solid rgba(0,0,0,0.6)', width: '30%', textAlign: 'center' }} className="site-btn">Back</Link>
										<Link onClick={handleFinishClick} className="site-btn" style={{ width: '30%', textAlign: 'center' }} >Finish</Link>
									</div>
									<div className="create-template-details">
										<div className="template-detail-top">
											<div className="template-top-circle">
										...
										<div className="template-detail-btn">
											<span onClick={() => { validateTabberChange('filter-social', true) }} data-filter="filter-cta" className="anchor-tabber site-btn-second-reverse">Back</span>
											<span onClick={handleFinishClick} style={{ float: 'right' }} data-filter="filter-cta" className="anchor-tabber site-btn-second">Finish</span>
										</div>
										{/* <div className="template-detail-btn" style={{display:'flex', alignItems:'center', justifyContent:'space-between', width:"100%"}}>
											...
											<div className="template-detail-first-btn">
												{
													loggerData?.isLoggedIn ?
														<div style={{display:'flex', flexDirection:'column',alignItems:'start', gap:'20px'}}>
															<span style={{padding:"14px 35px", backgroundColor:'#fff',boxShadow:'4px 4px 12px 4px rgba(0,0,0,0.11)',fontWeight:"500",  borderRadius:'10px'}} onClick={() => { validateTabberChange('filter-social', true) }} data-filter="filter-cta" className="anchor-tabber ">Continue Editing</span>
														...
														:
														<div style={{display:'flex', flexDirection:'column',alignItems:'start', gap:'20px'}}>
															<span style={{padding:"12px 25px", backgroundColor:'#fff', border:'1px solid #140342', borderRadius:'10px', display:'flex',alignItems:'center',gap:'10px' }} onClick={() => { validateTabberChange('filter-social', true) }} data-filter="filter-cta" className="anchor-tabber "><TiArrowBack style={{fontSize:'24px'}} /> <span>Continue Editing</span></span>
											...
											<div style={{display:'flex', alignItems:'start', justifyContent:'start', flexDirection:'column'}}>
												<div className="share-signature" style={{display:'flex', alignItems:'center', justifyContent:'flex-end', width:'auto', }} >
													...
													<span onClick={() => { copyWithStyle() }} id='copy-btn'>Copy Signature</span>
												</div>
								...
								<div>
									<div style={{ padding: '0px 0px 10px' }}>
										...
										<div className="comn-text-btn">
											<p id="finishModalBtn" style={{ cursor: "pointer" }} onClick={handleFinishRedirect}>Login To Finish!</p>
										</div>
									</div>
								</div>
